...
| **Jump / Vault**   | `SPACE`             | Jumps and lands on climbable bar, table, stool, sofa, and counter tops |
...
├── package.json            # Vite, quality, and visual QA script configuration
└── src/
    ├── main.js             # Entrypoint wiring the app controller into the browser
    ├── style.css           # Design system typography, cozy amber glows, and overlays
    ├── core/
    │   ├── AppController.js   # Application lifecycle, manager wiring, and disposal
    ...
    │   ├── LightingManager.js # Candle/gas lantern point lights and ambient fills
    │   └── Chandelier.js      # Grand slow-rotating wrought-iron candle chandelier
    ├── entities/
    │   └── NPCManager.js      # Procedural voxel character rigs & cozy behavior loops
    ├── ui/
    │   └── UIManager.js       # Entry modal, HUD, room prompts, and equalizer
    ├── config/
    │   └── experience.js      # Brand copy, room labels, and scripted tour metadata
    └── utils/
...
The package includes focused quality checks for code and formatting:
...
Use `npm run format` when you intentionally want Prettier to rewrite project files.
...
The underlying scripts remain available as `node take_screenshots.js`, `node record_tour.js`, and `node record_video.js` if you need to run them directly.

---

## ⚡ Technical Optimization Details

- **Capped Delta Frame rates**: Delta times are clamped to 100ms inside the `AppController` animation lifecycle. This prevents extreme coordinate displacement jumps (letting players walk through walls) if the browser tab loses focus or frame rate spikes.
