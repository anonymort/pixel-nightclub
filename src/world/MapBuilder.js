...
export class MapBuilder {
  ...
  /**
   * Generates procedurally compiled materials to style the club structures.
   */
  _initMaterials() {
    ...
    const floorGridTex = TextureGenerator.generateNeonGrid('#00f0ff', '#07060e', 128);
    floorGridTex.repeat.set(10, 10);

    const barGridTex = TextureGenerator.generateNeonGrid('#ff0055', '#09050d', 128);
    barGridTex.repeat.set(6, 4);

    this.speakerConeTex = TextureGenerator.generateSpeakerCone(128);
    this.djDecksTex = TextureGenerator.generateDJDecks(256);
    ...
    this.materials.danceFloorBorder = new THREE.MeshStandardMaterial({
    ...
    this.bottleMaterials = bottleColors.map(color => new THREE.MeshStandardMaterial({
      color: color,
      emissive: color,
    ...
    this.jacketMaterials = jacketColors.map(color => new THREE.MeshStandardMaterial({
      color: color,
      roughness: 0.85,
      metalness: 0.1
    }));
  ...
  /**
   * Assembles the complete architectural footprint of the club.
   */
  build() {
    this._buildFloors();
  ...
  _buildFloors() {
    ...
    for (let z = -13.0; z <= 13.0; z += 4.0) {
      this._buildBox(0.15, 0.01, 1.5, -17.8, -0.145, z, this.materials.roadMarkingYellow, false, false, true);
    }

    // 2. Lobby Floor (x: -4.8 to 1.0, z: -15 to 15)
    this._buildBox(5.8, 0.2, 30, -1.9, -0.1, 0, this.materials.lobbyFloor, false);

    // 3. Main Club Floor Border (x: 1.0 to 20.0, z: -10.5 to 11.0)
    // Floor is elevated slightly above 0 to prevent z-fighting with borders
    this._buildBox(19, 0.2, 21.5, 10.5, -0.1, 0.25, this.materials.danceFloorBorder, false);
    ...
    for (let c = 0; c < gridCols; c++) {
      for (let r = 0; r < gridRows; r++) {
        ...
        const tileMat = this.tileMaterials[Math.floor(Math.random() * this.tileMaterials.length)];
  ...
  /**
   * Places high boundaries around the nightclub layout.
   */
  _buildOuterWalls() {
    const wallH = 5.0;
    const halfH = wallH / 2;
  ...
  _buildInteriorWalls() {
    ...
    // B. Support Columns Framing Entrance (Solid exposed brick columns, no neon cyan/magenta lines)
    // Left Column
  ...
  /**
   * Builds the fully stocked Neon Bar counter and stool nodes.
   */
  _buildBarArea() {
    // Bar Counter shape (L-shape or long rectangular island)
    // Placed at z = 7.2. Spans x = 5.0 to 13.0
    this._buildBox(8.0, 1.1, 1.0, 9.0, 0.55, 7.5, this.materials.mahogany);
    ...
    for (let x = 5.8; x <= 12.2; x += 1.4) {
      // Gold base
      this._buildCylinder(0.18, 0.22, 0.1, x, 0.05, 5.8, this.materials.goldMetal, true);
      // Steel stem
      this._buildCylinder(0.05, 0.05, 0.5, x, 0.3, 5.8, this.materials.goldMetal, false);
      // Emerald velvet cushion top
      this._buildCylinder(0.2, 0.2, 0.12, x, 0.6, 5.8, this.materials.emeraldVelvet, true);
    }
    ...
    for (let shelfY = 1.26; shelfY <= 2.06; shelfY += 0.8) {
      for (let bx = 6.0; bx <= 12.0; bx += 0.6) {
        if (Math.random() > 0.3) {
          const bMat = this.bottleMaterials[Math.floor(Math.random() * this.bottleMaterials.length)];
  ...
  /**
   * Assembles the DJ booth, turntable console, and massive dual subwoofer towers.
   */
  _buildDJBooth() {
    // Acoustic Turntable Console Cabinet (centered at far end: x: 18, z: 0)
    this._buildBox(1.5, 1.15, 3.5, 17.5, 0.575, 0, this.materials.rusticOak);
  ...
  _buildChilloutLounge() {
    ...
    // Main cushions
    this._buildBox(5.0, 0.45, 1.2, 15.5, 0.225, -19.0, this.materials.sofaLeather, true); // Base seat
    ...
    this._buildBox(1.2, 0.45, 3.5, 8.5, 0.225, -15.5, this.materials.sofaLeather, true); // Seat
    ...
    this._buildBox(2.2, 0.35, 1.6, 13.0, 0.175, -15.0, this.materials.woodTable, true);
    ...
    // Seat Cushion
    this._buildBox(0.7, 0.35, 0.8, ac1X, 0.175, ac1Z, this.materials.loungeWool, true);
    ...
    // Seat Cushion
    this._buildBox(0.7, 0.35, 0.8, ac2X, 0.175, ac2Z, this.materials.loungeWool, true);
    ...
    loungeCorners.forEach(corner => {
      ...
      this._buildBox(0.9, 0.08, 0.3, corner.px + 0.4, 2.0, corner.pz, this.materials.plantGreen, false);
      this._buildBox(0.9, 0.08, 0.3, corner.px - 0.4, 2.0, corner.pz, this.materials.plantGreen, false);
      this._buildBox(0.3, 0.08, 0.9, corner.px, 2.0, corner.pz + 0.4, this.materials.plantGreen, false);
      this._buildBox(0.3, 0.08, 0.9, corner.px, 2.0, corner.pz - 0.4, this.materials.plantGreen, false);
    });
    ...
    // Matte card mounting
    this._buildBox(0.02, 1.2, 1.2, frameX - 0.015, frameY, frameZ, this.materials.loungeWool, false);
    // Botanical print
    const botanicalPosterMat = new THREE.MeshStandardMaterial({ map: TextureGenerator.generatePoster('BOTANICAL', 128) });
    ...
    sconces.forEach(s => {
      ...
      this._buildBox(0.04, 0.06, 0.12, s.sx, s.sy - 0.1, s.sz + 0.08, this.materials.goldMetal, false);
      // White candle cylinder
      this._buildBox(0.08, 0.18, 0.08, s.sx, s.sy + 0.05, s.sz + 0.12, this.materials.loungeWool, false);
      // Glowing orange flame
      this._buildBox(0.04, 0.08, 0.04, s.sx, s.sy + 0.16, s.sz + 0.12, this.materials.glowOrange, false);
    });
  }
  ...
  _buildDecorations() {
    ...
    const posterMat1 = new THREE.MeshStandardMaterial({ map: TextureGenerator.generatePoster('RAVE', 128) });
    const posterMat2 = new THREE.MeshStandardMaterial({ map: TextureGenerator.generatePoster('VIP', 128) });
    const posterMat3 = new THREE.MeshStandardMaterial({ map: TextureGenerator.generatePoster('DJSYNC', 128) });
  ...
  _buildCloakroom() {
    ...
    this._buildBox(1.0, 1.1, 3.2, -1.6, 0.55, 6.0, this.materials.mahogany, true);
    ...
    for (let sy = 0.3; sy <= 2.1; sy += 0.6) {
      for (let sz = 4.4; sz <= 7.6; sz += 0.6) {
        if (Math.random() > 0.2) {
          const jacketMat = this.jacketMaterials[Math.floor(Math.random() * this.jacketMaterials.length)];
          this._buildBox(0.3, 0.35, 0.45, 0.35, sy + 0.175, sz, jacketMat, false);
        }
      }
    }

    // 3. Cozy bench seating along the opposite wall
    this._buildBox(0.8, 0.3, 3.0, -4.2, 0.15, 6.0, this.materials.woodTable, true);
    // Cozy wool cushions (instead of neon cyan)
  ...
  _buildClassicCar() {
    ...
    this._buildBox(1.9, 0.55, 4.0, tx, ty + 0.525, tz, this.materials.racingGreen, false, true, true);

    // C. Upper Cabin / Windshield (black glass / chrome trim framing)
    this._buildBox(1.7, 0.45, 2.2, tx, ty + 1.025, tz - 0.1, this.materials.taxiBlack, false, true, true);
    ...
    // Front-left
    this._buildBox(wheelW, wheelH, wheelD, tx - wheelOffsetX, ty + 0.275, tz - wheelOffsetZ, this.materials.taxiBlack, false);
    this._buildBox(0.04, 0.3, 0.3, tx - wheelOffsetX - 0.16, ty + 0.275, tz - wheelOffsetZ, this.materials.chrome, false);
    // Front-right
    this._buildBox(wheelW, wheelH, wheelD, tx + wheelOffsetX, ty + 0.275, tz - wheelOffsetZ, this.materials.taxiBlack, false);
    this._buildBox(0.04, 0.3, 0.3, tx + wheelOffsetX + 0.16, ty + 0.275, tz - wheelOffsetZ, this.materials.chrome, false);
    // Rear-left
    ...
    this._buildBox(0.02, 0.05, 3.8, tx - 0.96, stripY, tz, this.materials.goldTrim, false, false, true);
    this._buildBox(0.02, 0.05, 3.8, tx + 0.96, stripY, tz, this.materials.goldTrim, false, false, true);
    ...
    this._buildBox(0.2, 0.2, 0.05, tx - 0.6, headlightY, headlightZ, this.materials.neonCyan, false);
    this._buildBox(0.2, 0.2, 0.05, tx + 0.6, headlightY, headlightZ, this.materials.neonCyan, false);

    // H. Glowing Taillights (red) at the rear
    const taillightZ = tz + 2.02;
    const taillightY = ty + 0.45;
    this._buildBox(0.2, 0.12, 0.05, tx - 0.6, headlightY, taillightZ, this.materials.glowRed, false);
    this._buildBox(0.2, 0.12, 0.05, tx + 0.6, headlightY, taillightZ, this.materials.glowRed, false);
  ...
  _buildRoof() {
    ...
    this._buildBox(5.8, thickness, 30.0, -1.9, ceilingY + thickness / 2, 0, ceilingMat, false, true, true);

    // 2. Main Club Floor & Bar Ceiling Slab (x: 1.0 to 20.0, z: -10.5 to 11.0)
    // Width = 19.0, Depth = 21.5, Center = (10.5, 5.075, 0.25)
    this._buildBox(19.0, thickness, 21.5, 10.5, ceilingY + thickness / 2, 0.25, ceilingMat, false, true, true);

    // 3. Chillout Lounge Ceiling Slab (x: 6.0 to 20.0, z: -21.0 to -10.5)
    // Width = 14.0, Depth = 10.5, Center = (13.0, 5.075, -15.75)
    this._buildBox(14.0, thickness, 10.5, 13.0, ceilingY + thickness / 2, -15.75, ceilingMat, false, true, true);
    ...
    for (let z = -12.0; z <= 12.0; z += 3.0) {
      this._buildBox(5.8, beamDepth, beamWidth, -1.9, ceilingY - beamDepth / 2, z, beamMat, false, true, true);
    }

    // B. Main Club Floor Beams (X-aligned, spaced every 2.5m along Z)
    for (let z = -10.0; z <= 10.0; z += 2.5) {
      this._buildBox(19.0, beamDepth, beamWidth, 10.5, ceilingY - beamDepth / 2, z, beamMat, false, true, true);
    }

    // C. Chillout Lounge Beams (Z-aligned, spaced every 2.5m along X)
    for (let x = 7.5; x <= 18.5; x += 2.5) {
      this._buildBox(beamWidth, beamDepth, 10.5, x, ceilingY - beamDepth / 2, -15.75, beamMat, false, true, true);
    }
  }
}
