...
The application is engineered using a highly decoupled, modular, and event-driven manager pattern. A central entry point manages system state and drives a coordinated, frame-capped ticks system.

```mermaid
graph TD
    A[main.js] --> B[SceneManager]
    A --> C[AudioManager]
    A --> D[ControlsManager]
    A --> E[MapBuilder]
    A --> F[LightingManager]
...
1. **Bootstrap Phase**: `main.js` is loaded as an ES module. It instantiates the `SceneManager` to set up the WebGL context, the `AudioManager` to initialize audio parameters (in a suspended state), and the `ControlsManager` to listen for keyboard and mouse events.
2. **Construction Phase**: `MapBuilder` compiles materials, builds geometry, and registers bounding boxes (`THREE.Box3`) with `ControlsManager`. `LightingManager`, `DiscoBall` (Chandelier), `NPCManager`, and `UIManager` are initialized, referencing their corresponding dependencies.
3. **Registration Phase**: Updatable nodes are registered inside `SceneManager`.
4. **Interaction Trigger**: When the player clicks **"ACCEPT ADMISSION"** on the parchment ticket overlay, the Web Audio Context is started. The `UIManager` hides the overlay and requests pointer lock on the canvas, kicking off player navigation and procedural track playback.
5. **Tick Phase**: `requestAnimationFrame` drives the game loop in `main.js`. It calculates the frame delta ($dt$), clamps it to a maximum of $100\text{ms}$ to prevent physics tunnel-through on tab blur, and tells `SceneManager` to run `update()` on all registered modules.

---

## 🛠️ Subsystem Deep Dives

### 1. SceneManager (`src/core/SceneManager.js`)
...
### 2. ControlsManager (`src/core/ControlsManager.js`)
...
### 3. AudioManager (`src/core/AudioManager.js`)
...
### 4. TextureGenerator (`src/utils/TextureGenerator.js`)
...
* **Dynamic Canvas Rasterization**:
  ...
  - `generateFramedArt()`: Replaces neon posters with beautiful hand-drawn botanical sketches, forest silhouettes, and warm landscape scenes inside polished mahogany frames.

---

### 5. MapBuilder (`src/world/MapBuilder.js`)
...
* **Oak Trellis Pergola**: The entrance features a beautiful rustic oak wooden trellis pergola adorned with trailing green ivy vines, replacing high-tech neon awnings.
...
### 6. LightingManager (`src/world/LightingManager.js`)
...
### 7. Grand Candle Chandelier (`src/world/DiscoBall.js`)
...
### 8. NPCManager (`src/entities/NPCManager.js`)
...
* **Execution Flow**:
  ...
  4. Automatically relocates the player camera and target vector to 5 key positions: **Exterior Facade**, **Cloakroom Reception Desk**, **Botanist Bar**, **Fireplace Lounge**, and the **Acoustic Hall (Dancefloor)**.
...
Captures a fully continuous, high-definition (720p @ 30fps) walking video walkthrough of the club using the HTML5 `MediaRecorder` API directly on the canvas stream.
...
3. **Disposal & Garbage Collection**:
   All entities implement a `dispose()` method. When destroyed, they close Web Audio contexts, stop timers, and clear intervals, ensuring there are no memory leaks.
...
* [ ] Verify that clicking the "ACCEPT ADMISSION" button activates audio playback and hides the ticket modal.
